...
const Item = ({image}: Props) => {
    let items = [];
    for (let i = 0; i < 5; i++) {
        items.push(<svg key={i} aria-hidden="true"
    ...
    return (
        <div
            className="w-full max-w-md h-auto bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
            <Link href="#">
                <img className="rounded-t-lg pb-2 w-full h-auto" src={image} alt="product image"/>
            </Link>
